-- Migration script to fix foreign key constraints by dropping and recreating tables
-- This safely migrates from the old schema to the auth.users compatible schema

-- STEP 1: Drop all existing tables in correct order (respecting foreign key dependencies)
...
DROP TABLE IF EXISTS users CASCADE;  -- Remove the problematic users table
...
-- STEP 3: Now create the new schema (same as schema-fixed-auth.sql)
